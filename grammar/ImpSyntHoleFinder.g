tree grammar ImpSyntHoleFinder;
...
statement
    :   (whileLoop {bbSize = 0;} | conditional | command {bbSize++;} | ^(ATOMSTMT methodBody))
    ;
...
command
  ...
  |   functionCall
  |   ret
  |   unknownStmt {unknownStmts.add($unknownStmt.tree);}
  ;
...
atomIntExp returns [CommonTree value]
    :   ^(CONS INT)
  |   iVar
  ;
...
locExp
    :   NIL
  |   NEW {unknownNew.add(new Integer(1));}
  |   lVar
  ;
...
atomBoolExp returns [CommonTree value]
    : ^(ATOMBOOL boolExp)
  | ^(LOCEQ locExp locExp)
  | ^(INTEQ intExp intExp)
  | ^('<' intExp intExp)
  ...
  | TRUE
  | bVar
  | ^('cond' atomIntExp) {unknownConds.add($atomIntExp.tree);}
    ;
...
iVar returns [String value]
    :   ^(IVAR ID)
    |  ^(IVAR a=QMARK) {unknownInts.add(new Integer(order++));}
    ;

lVar returns [String value]
    :   ^(LVAR ID)
    |  ^(LVAR a=QQMARK) {unknownLocs.add(new Integer(order++));}
    ;
